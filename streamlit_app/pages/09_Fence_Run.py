...
with col1:
    ...
    st.number_input("Total Run Length (ft)", key="fr_total_length",
        min_value=10.0, max_value=10000.0, step=10.0)
    st.number_input("Post Spacing (ft)", key="fr_post_spacing",
        min_value=4.0, max_value=20.0, step=0.5)
    st.number_input("Number of Corners", key="fr_num_corners",
        min_value=0, max_value=20, step=1)
    st.number_input("Number of Gates", key="fr_num_gates",
        min_value=0, max_value=20, step=1)

with col2:
    st.subheader("Post & Footing")
    st.number_input("Post Height (ft)", key="fr_post_height",
        min_value=3.0, max_value=20.0, step=0.5)
    st.number_input("Post Weight (plf)", key="fr_post_weight",
        min_value=0.1, max_value=30.0, step=0.1)
    st.number_input("Footing Diameter (ft)", key="fr_footing_diam",
        min_value=0.5, max_value=5.0, step=0.25)
    st.number_input("Fabric Height (ft)", key="fr_fabric_height",
        min_value=1.0, max_value=20.0, step=0.5)
    st.checkbox("Include Top Rail", key="fr_has_top_rail")

# Gate widths
gates = []
num_gates = st.session_state.fr_num_gates
if num_gates > 0:
    ...
    for i in range(int(num_gates)):
        with gate_cols[i % len(gate_cols)]:
            ...
            gate_key = f"gate_w_{i}"
            if gate_key not in st.session_state:
                st.session_state[gate_key] = 10.0
            st.number_input(f"Gate {i+1} Width (ft)",
                min_value=3.0, max_value=40.0, step=0.5, key=gate_key)
            gates.append(GateSpec(width_ft=st.session_state[gate_key]))

# Footing depths
with st.expander("Footing Depths (per post type)"):
    fc1, fc2, fc3 = st.columns(3)
    with fc1:
        st.number_input("Line Post Depth (ft)", key="fr_depth_line",
            min_value=1.0, max_value=12.0, step=0.25)
    with fc2:
        st.number_input("Pull Post Depth (ft)", key="fr_depth_pull",
            min_value=1.0, max_value=12.0, step=0.25)
    with fc3:
        st.number_input("Gate Post Depth (ft)", key="fr_depth_gate",
            min_value=1.0, max_value=12.0, step=0.25)
...
if st.button("Calculate Quantities", type="primary"):
    inp = FenceRunInput(
        total_length_ft=st.session_state.fr_total_length,
        post_spacing_ft=st.session_state.fr_post_spacing,
        num_corners=int(st.session_state.fr_num_corners),
        gates=gates,
        post_height_ft=st.session_state.fr_post_height,
        post_weight_plf=st.session_state.fr_post_weight,
        footing_diameter_ft=st.session_state.fr_footing_diam,
        footing_depth_line_ft=st.session_state.fr_depth_line,
        footing_depth_pull_ft=st.session_state.fr_depth_pull,
        footing_depth_gate_ft=st.session_state.fr_depth_gate,
        fabric_height_ft=st.session_state.fr_fabric_height,
        has_top_rail=st.session_state.fr_has_top_rail,
    )
